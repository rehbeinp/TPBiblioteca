...
if($_SESSION['status'] != "" ){
    ...
    $sql_retirados = "SELECT 
            ...
            where u.email='$user'
            order by e.data_retirada, e.data_devolucao_prevista desc";
           ...
           if ($num_linhas >= 1) {
            ...
            while ($resul = mysqli_fetch_assoc($exesql_retirados)) {
                ...
                if( $resul['valor_multa'] == null){
                    ...
                    echo "<strong>Motivo da Multa: </strong>" . $resul['motivo_multa']. " <br>";
                    if($resul['paga'] == 1) {echo "<strong> Multa quitada :) </strong> <br><br>";}
                    else{ echo "<strong> Multa em haver :( </strong> <br> Venha pagar para não aumentar ;) <br><br> ";}
                }
            }
